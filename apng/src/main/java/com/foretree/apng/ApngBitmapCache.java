 ...
 *
 * 1. 有效控制缓存大小. 只存储解码所需的缓存帧(在apng解包时根据op类型, 可以计算出解码过程中最多需要的缓存数),
 * 理论上最少缓存帧数为1帧, 一般2-3帧就可以了
 * 2. 复用Bitmap. 所有使用Bitmap的地方均从bitmap复用池中获取, 不再需要的Bitmap也都放入复用池中。
 * 从目前的测试结果来看, 复用池的大小一般在1-2之间变化
 */
